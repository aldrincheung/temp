...
public class Ticket implements Comparable<Ticket>{
    ...
    public int compareTo(Ticket other){
        Integer orderCurrent = orderMap.get(getTicketType());
        Integer orderOther = orderMap.get(other.getTicketType());
        int orderDiff = orderCurrent.compareTo(orderOther);


        Long timeCurrent = getCreatedAt();
        Long timeOther = other.getCreatedAt();
        int timeDiff = timeCurrent.compareTo(timeOther);

        if(orderDiff != 0) return orderDiff;
        else if(timeDiff != 0) return (int) timeDiff;
        else return 0;
    }
